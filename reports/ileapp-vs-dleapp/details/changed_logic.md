...
- only in dleapp: `ipgen` — `def ipgen(report_folder, data_list_ipaddress)`
- only in dleapp: `logdevinfo` — `def logdevinfo(message=...)`
- logic changed: `tsv`
- only in dleapp: `usergen` — `def usergen(report_folder, data_list_usernames)`
